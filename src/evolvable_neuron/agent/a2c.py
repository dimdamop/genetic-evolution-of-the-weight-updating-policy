...
class Agent:
    ...
    def a2c_loss(
        ...
        parametric_action_distribution = self.ac_models.parametric_action_distribution
    ...
    def rollout(
        ...
        def run_one_step(
            ...
            episode_count = acting_state.episode_count + jax.lax.psum(
                next_timestep.last().sum(), "devices"
            )
            env_step_count = acting_state.env_step_count + jax.lax.psum(
                self.batch_size_per_device, "devices"
            )
            acting_state = ActingState(
