...
"""Implementation of some standard optimizers in JAX.
...
.. code-block:: python
    ...
    def init_fun(params):
        ...
        Returns:
            A pytree representing the initial optimizer state, which includes its initial parameters
            and may also include auxiliary values like initial momentum. The optimizer state pytree
            structure differs in general from that of `params`.
        '''
        pass
...
.. code-block:: python
    ...
    def update_fun(step, grads, opt_state):
        ...
        Args:
            ...
            grads: a pytree with the same structure as `get_params(opt_state)` representing the
                gradients to be used in updating the optimizer state.

            opt_state: a pytree representing the optimizer state to be updated.

        Returns:
            A pytree with the same structure as the `opt_state` argument representing the updated
            optimizer state.
        '''
        pass
...
def optimizer(
    ...
    Returns:
        ...
        `ParameterPytree (OptStatePytree ndarray)`, but may store the state instead as eg.
        a partially-flattened data structure for performance.
    """
    ...
    def tree_opt_maker(*args, **kwargs):
        ...
        def tree_update(i, grad_tree, opt_state):
            ...
            if tree2 != tree:
                msg = (
                    "optimizer update function was passed a gradient tree that did not match the "
                    "parameter tree structure with which it was initialized: parameter tree {} and "
                    "grad tree {}."
                )
            ...
            for subtree, subtree2 in zip(subtrees, subtrees2):
                if subtree2 != subtree:
                    msg = (
                        "optimizer update function produced an output structure that did not match "
                        "its input structure: input {} and output {}."
                    )
...
def sgd(step_size):
    ...
    def init(x0):
        """There is no "auxiliary" state in vanilla SGD: just that of the parameters of the
        network.
        """
        return x0
...
def adagrad(step_size, momentum=0.9):
    ...
    def update(i, g, state):
        x, g_sq, m = state
        g_sq += jnp.square(g)
        g_sq_inv_sqrt = jnp.where(g_sq > 0, 1.0 / jnp.sqrt(g_sq), 0.0)
...
def rmsprop(step_size, gamma=0.9, eps=1e-8):
    ...
    def update(i, g, state):
        x, avg_sq_grad = state
        avg_sq_grad = avg_sq_grad * gamma + jnp.square(g) * (1.0 - gamma)
...
def adam(step_size, b1=0.9, b2=0.999, eps=1e-8):
    ...
    Args:
        ...
        b1: optional, a positive scalar value for beta_1, the exponential decay rate for the first
            moment estimates (default 0.9).

        b2: optional, a positive scalar value for beta_2, the exponential decay rate for the second
            moment estimates (default 0.999).

        eps: optional, a positive scalar value for epsilon, a small constant for numerical stability
            (default 1e-8).
...
def adamax(step_size, b1=0.9, b2=0.999, eps=1e-8):
    ...
    def update(i, g, state):
        ...
        x = x - (step_size(i) / (1 - jnp.asarray(b1, m.dtype) ** (i + 1))) * m / (u + eps)
        return x, m, u
...
def sm3(step_size, momentum=0.9):
    ...
    def update(i, g, state):
        ...
        accum_inv_sqrt = jnp.where(accum > 0, 1.0 / jnp.sqrt(accum), 0)
...
def make_schedule(scalar_or_schedule: Union[float, Schedule]) -> Schedule:
    ...
    if jnp.ndim(scalar_or_schedule) == 0:
...
def unpack_optimizer_state(opt_state):
    ...
    Converts an OptimizerState to a marked pytree with the leaves of the outer pytree represented as
    JoinPoints to avoid losing information. This function is intended to be useful when serializing
    optimizer states.
...
def pack_optimizer_state(marked_pytree):
    ...
    The inverse of unpack_optimizer_state. Converts a marked pytree with the leaves of the outer
    pytree represented as JoinPoints back into an OptimizerState. This function is intended to be
    useful when deserializing optimizer states.
